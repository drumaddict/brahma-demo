module SSHKit
  ...
  module Formatter
    ...
    class Kabasakalis < SSHKit::Formatter::Pretty

    LEVEL_NAMES = %w{ DEBUG INFO WARN ERROR FATAL }.freeze
      LEVEL_COLORS = [:yellow, :yellow, :yellow, :yellow, :yellow].freeze
      ...
      def colorize(obj, _color, _mode=nil)
        ...
        level = colorize(Pretty::LEVEL_NAMES[verbosity], Kabasakalis::LEVEL_COLORS[verbosity])
        '%6s %s' % [level, message]
      end
      ...
      def log_command_start(command)
        ...
        message = "Runningdfdfdfd #{colorize(command, :black, :bold)} #{host_prefix}#{colorize(command.host, :blue)}"
